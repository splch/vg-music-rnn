...
songs = get_songs('VG_Music')  # These songs have already been converted from midi to msgpack
...
batch_size = 1000  # The number of training examples that we are going to send through the RBM at a time.
...
W_adder = tf.mul(lr / size_bt,
                      tf.sub(tf.matmul(tf.transpose(x), h), tf.matmul(tf.transpose(x_sample), h_sample)))
...
with tf.Session() as sess:
    ...
    sample = gibbs_sample(1).eval(session=sess, feed_dict={x: np.zeros((20, n_visible))})
